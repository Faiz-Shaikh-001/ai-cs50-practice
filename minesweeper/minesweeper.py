...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Add sentence to knowledge base including all surrounding cells
        cells = set()
        for i in range(cell[0]-1, cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                curr_cell = (i, j)
                if self.in_bound(curr_cell) and not self.in_safes_or_mines(curr_cell):
                    cells.add(curr_cell)

        self.knowledge.append(Sentence(cells, count))
        self.update_known_mines_safes()
        self.clear_knowledge_base()
        self.inferr_knowledge()
            ...
            for j in range(i, len(self.knowledge)):
                ...
                if len(set1.cells) < len(set2.cells):
                    if set1.cells.issubset(set2.cells):
                        set3 = (set2.cells - set1.cells, set2.count - set1.count)
                        inferred_knowledge.append(Sentence(set3[0], set3[1]))
                    continue
                if len(set2.cells) < len(set1.cells):
                    if set2.cells.issubset(set1.cells):
                        set3 = (set1.cells - set2.cells, set1.count - set2.count)
                        inferred_knowledge.append(Sentence(set3[0], set3[1]))
    ...
    def update_known_mines_safes(self):
        for sentence in self.knowledge:
            mines_to_mark = set()
            safes_to_mark = set()
    ...
    def in_bound(self, cell):
            i, j = cell
            return 0 <= i < self.height and 0 <= j < self.width

    def in_safes_or_mines(self, cell):
